...
var ContentServicesPage = function (){
    ...
    var currentFolder = element(by.css("li[class*='adf-breadcrumb-item active'] div"));
    var createFolderButton = element(by.cssContainingText("mat-icon", "create_new_folder"));
    var activeBreadcrumb = element(by.css("li[class*='active']"));
    ...
    var contentServicesURL = TestConfig.adf.base + TestConfig.adf.adf_port + "/files";
    ...
    this.getBreadcrumbTooltip = function (content) {
        return element(by.css("div[data-automation-id='breadcrumb'] li[title='" +content +"']")).getAttribute('title');
    };
    ...
    this.uploadFile = function (fileLocation) {
        ...
        uploadFileButton.sendKeys(path.resolve(path.join(TestConfig.adf.rootPath, fileLocation)));
        this.checkUploadButton();
        return this;
     };
    ...
    this.uploadMultipleFile = function (files) {
        ...
        var allFiles = path.resolve(path.join(TestConfig.adf.rootPath, files[0]));
        for(var i =1; i< files.length; i++) {
            allFiles = allFiles + "\n" + path.resolve(path.join(TestConfig.adf.rootPath, files[i]));
        };
    ...
    this.uploadFolder = function (folder) {
        ...
        uploadFolderButton.sendKeys(path.resolve(path.join(TestConfig.adf.rootPath, folder)));
        Util.waitUntilElementIsVisible(uploadFolderButton);
        return this;
    };
