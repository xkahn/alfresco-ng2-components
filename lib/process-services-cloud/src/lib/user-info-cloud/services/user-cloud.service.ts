...
export class UserCloudService {
    ...
    getCurrentUserInfo(): Observable<UserCloud> {
        const fullName = this.getValueFromToken<string>(UserCloudService.USER_NAME);
        const email = this.getValueFromToken<string>(UserCloudService.USER_EMAIL);
        const nameParts = fullName.split(' ');
        const user = { firstName: nameParts[0], lastName: nameParts[1], email: email };
        return of(new UserCloud(user));
    }

    getValueFromToken<T>(key: string): T {
        let value;
        const token = localStorage.getItem(UserCloudService.USER_ACCESS_TOKEN);
        if (token) {
